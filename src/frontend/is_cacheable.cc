...
int main( int argc, char *argv[] )
{
    try {
        if ( argc < 2 ) {
            throw runtime_error( "Usage: " + string( argv[ 0 ] ) + " proto_file" );
        }

        string proto_file = argv[ 1 ];

        FileDescriptor fd( SystemCall( "open protobuf file", open( proto_file.c_str(), O_RDONLY ) ) );
        ...
        if ( cacheable ) {
            if ( freshness > 0 ) {
                cout << "cacheable freshness=" << freshness << " header=" << header_used << endl;
            }
            // else {
